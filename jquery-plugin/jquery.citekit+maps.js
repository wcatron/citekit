(function( $ ) {
    ...
    $.fn.ckLoadMaps = function(options) {
 	   	var opts = $.extend( {}, $.fn.ckLoadMaps.defaults, options);
		  ...
		  var styles = [
  {
    ...
    "stylers": [
      { "visibility": "off" }
		...
		this.gmap({styles:styles});
		this.css("position","fixed");
    };
	
	$.fn.ckLoadLocations = function(elm) {	
		var map_controller = this;	
		if ( elm.attr("cite")) {
			thisURN = elm.attr("cite");
			// Currently only works with the folio service
			$.getJSON( "http://folio.furman.edu/fys/api?urn="+thisURN+"&type=pleiades", function( data ) {
			  $.each( data.results.bindings, function( index, binding ) {
				  console.log("urn: " +binding.urn.value + " geo:" + binding.geo.value);
				  $.getJSON(binding.geo.value, function( data ) {
					  console.log(data);
					  addMarker(map_controller,data.reprPoint[1],data.reprPoint[0],data.title);
					  
					  var book_line = lineNumber(thisURN,binding.urn.value);
					  if (book_line) {
					  		console.log(book_line);
							line_elm = elm.find("div p")[book_line];
						  	$(line_elm).addClass("has_location");
							$(line_elm).append("Location: "+data.title);
				  		}	
					  //console.log();
				  });
			  });
	...
	function lineNumber(blockquote_urn, location_urn) {
		bq_book_line = getBookLine(blockquote_urn);
		loc_book_line = getBookLine(location_urn);
		
		if (bq_book_line[0]-loc_book_line[0] != 0) {
			console.log('WARNING: The results could be inaccurate due to cross book line location searching.');
			return false;
		}
		
		return parseInt(loc_book_line[1])-parseInt(bq_book_line[1])+1;//plus 1 because it's not 0-n 1-n
	}
	
	function getBookLine(urn) {
		var urn_data = urn.split(':');
		return urn_data[urn_data.length-1].split('.');
	}
	
	function addMarker(elm, lat, long, name) {
		elm.gmap('addMarker', {'position': lat+','+long, 'bounds': true}).click(function() {
			elm.gmap('openInfoWindow', {'content': name}, this);
		});
	}
	
	$.fn.ckLoadMaps.defaults = {
